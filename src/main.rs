...
fn main() {
    ...
    let (info, info_personnes) = read_sheet(&mut reponses, &sheets);
    ...
    init_sheet(local, &sheets, MOIS, ANNEE, &liste_noms);

    let b00 = workbook.add_worksheet()
        .set_name("B00")
        .expect("Impossible de renommer la feuille \"B00\"");
    init_sheet(b00, &sheets, MOIS, ANNEE, &liste_noms);
    ...
    init_sheet(personnes, &sheets, MOIS, ANNEE, &liste_noms);
