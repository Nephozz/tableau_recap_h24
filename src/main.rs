...
use rust_xlsxwriter::{Format, Workbook, FormatAlign, Worksheet, XlsxColor};

pub fn init_table(worksheet: &mut Worksheet, sheets: &Vec<&String>, mois: &str, annee: &str) {
    ...
    let titre: String = "Accès TVn7 ".to_owned() + mois + " " + annee + " " +"(" + &worksheet.name() + ")";

    worksheet.merge_range(0, 1, 0, 30, &titre, &format).unwrap();
}

fn main() {
    let path: &'static str = "C:/Users/thoma/OneDrive/Documents/Internet/H24/05-2023 (réponses).xlsx";
    let reponses: Xlsx<_> = open_workbook(path).expect("Impossible d'ouvrir le fichier !");
    ...
    workbook.save("Accès ".to_owned() + mois + ".xlsx").unwrap();
}